...
We picked up our data set from Kaggle -- the data science competition
web site, because it is recommended in the lecture.  The problem is
...
code and the data privately.  The former is circumvented, because
the sharing code is not allowed privately, but is okay publicly. When
...
data. But I think it is too much to ask for to reviewers. So, my code
would not run without the data. The only option I can think of is that
I will give up 20 points of score that comes with the code. My hope is
still there is a chance to pass the course, if I am lucky with other
part. I put all the output of the code in my GitHub repo, including
the intermediate products and the submission file, trying to give you
some proof that the code actually runs.

One more thing: the problem described in the following sections is not
a predictive type, such as we have training and test data set, and
construct some fitting functions using training data. This is
optimisation problem, and one of the variants of the traveling
...
like clustering, but it is unsupervised learning, more toward the
...
The final score is 427247. This is about the middle of 
all contestant in the competition. 
...
The spots where the most of the loss happen
are shown below. The fractional loss becomes 
larger toward the end. 
